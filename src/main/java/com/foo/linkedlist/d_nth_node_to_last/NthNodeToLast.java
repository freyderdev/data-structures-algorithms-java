...
public class NthNodeToLast {
    ...
    public Node nthNodeToLast(Node head, int n) {
        ...
        // ⚠️ Validación: si n es inválido (mayor que la longitud o menor o igual a 0), no hay solución
        if (n > length || n <= 0) {
    ...
    /**
     * Devuelve el nodo que está en la posición N contando desde el final de la lista,
    ...
    public Node nthNodeToLastWithPointers(Node head, int n) {
        ...
        for (int i = 0; i < n; i++) {
            if (fast == null) return null;// n es mayor que la longitud
            fast = fast.next;
        }
